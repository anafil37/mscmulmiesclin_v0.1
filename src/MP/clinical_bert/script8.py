...
def train(
    ...
    for epoch_i in range(epochs):
        ...
        for step, (b_input_ids, b_attn_mask, b_labels) in enumerate(train_dataloader):
            ...
            # Reshape hidden_state with a linear layer
            linear_layer = nn.Linear(hidden_state.shape[-1], 1248)
            hidden_state_tensor = linear_layer(hidden_state[:, 0, :])

            loss = criterion(hidden_state_tensor, b_labels)
...
def evaluate(model, val_dataloader):
    ...
    with torch.no_grad():
        ...
        for b_input_ids, b_attn_mask, b_labels in val_dataloader:
            ...
            logits = model(**inputs)
            hidden_state = logits.last_hidden_state
            ...
            loss = crit(hidden_state_tensor, b_labels)

            val_loss.append(loss.item())

            # Get the predictions
            # preds = torch.argmax(logits, dim=1).flatten()
            _, preds = torch.max(hidden_state_tensor, 1)
